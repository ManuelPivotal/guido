 ...
 */
package oss.guido.ch.qos.logback.classic.turbo;

import static oss.guido.ch.qos.logback.core.CoreConstants.MILLIS_IN_ONE_SECOND;
...
import oss.guido.ch.qos.logback.core.status.StatusUtil;
import oss.guido.org.slf4j.Marker;
import oss.guido.ch.qos.logback.core.joran.event.SaxEvent;
import oss.guido.ch.qos.logback.core.joran.spi.ConfigurationWatchList;
import oss.guido.ch.qos.logback.core.joran.spi.JoranException;
import oss.guido.ch.qos.logback.core.joran.util.ConfigurationWatchListUtil;
import oss.guido.ch.qos.logback.core.spi.FilterReply;
...
public class ReconfigureOnChangeFilter extends TurboFilter {
	...
	public void start() {
		...
		if (configurationWatchList != null) {
			...
			List<File> watchList = configurationWatchList
	...
	public String toString() {
		return "ReconfigureOnChangeFilter{" + "invocationCounter="
				+ invocationCounter + '}';
	}
	...
	public FilterReply decide(Marker marker, Logger logger, Level level,
		...
		// for performance reasons, skip change detection (MASK-1) times out of
		// MASK.
		...
		synchronized (configurationWatchList) {
			...
			if (changeDetected(now)) {
				// Even though reconfiguration involves resetting the
				// loggerContext,
				// which clears the list of turbo filters including this
				// instance, it is
				// still possible for this instance to be subsequently invoked
				// by another
				// thread if it was already executing when the context was
				// reset.
	...
	// if less than MASK_INCREASE_THRESHOLD milliseconds elapse between
	// invocations of updateMaskIfNecessary() method,
	// then the mask should be increased
	private static final long MASK_INCREASE_THRESHOLD = 100;

	// if more than MASK_DECREASE_THRESHOLD milliseconds elapse between
	// invocations of updateMaskIfNecessary() method,
	// then the mask should be decreased
	private static final long MASK_DECREASE_THRESHOLD = MASK_INCREASE_THRESHOLD * 8;

	// update the mask so as to execute change detection code about once every
	// 100 to 8000 milliseconds.
	private void updateMaskIfNecessary(long now) {
		final long timeElapsedSinceLastMaskUpdateCheck = now - lastMaskCheck;
		lastMaskCheck = now;
		if (timeElapsedSinceLastMaskUpdateCheck < MASK_INCREASE_THRESHOLD
				&& (mask < MAX_MASK)) {
			mask = (mask << 1) | 1;
	...
	class ReconfiguringThread implements Runnable {
		public void run() {
			...
			addInfo(CoreConstants.RESET_MSG_PREFIX + "named ["
					+ context.getName() + "]");
	...
	private void fallbackConfiguration(LoggerContext lc,
			List<SaxEvent> eventList, URL mainURL) {
		...
		// try {
		// lc.reset();
		// joranConfigurator.informContextOfURLUsedForConfiguration(context,
		// mainURL);
		// joranConfigurator.doConfigure(eventList);
		// addInfo("Re-registering previous fallback configuration once more as a fallback configuration point");
		// joranConfigurator.registerSafeConfiguration();
		// } catch (JoranException e) {
		// addError("Unexpected exception thrown by a configuration considered safe.",
		// e);
		// }
		// } else {
